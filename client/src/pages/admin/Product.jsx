...
const Product = () => {
  ...
  const columns = [
    {
      ...
      render: (text, record) => (
        <img
          src={record.imageUrl ? (record.imageUrl.startsWith('https') ? record.imageUrl : `${import.meta.env.VITE_BACKEND_TARGET}/storage/product/${record.imageUrl}`) : product_default}
    ...
    },
    {
      ...
      key: 'details',
      render: (_, record) => (
        <Button type="link" onClick={() => handleShowMessage(record.description, record.product_name)}>
          Xem chi tiết
        </Button>
      ),
    },
  ...
  return (
    <div className="w-full pr-3 relative">
      <Table dataSource={products?.data?.result} columns={columns} rowKey="id" pagination={{
        current: currentPage,
